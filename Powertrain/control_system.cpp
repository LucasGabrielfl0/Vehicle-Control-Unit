...
:APPS_1{_apps_1},
 APPS_2{_apps_2},
 BSE_sensor{_bse_sensor}, 
 Steering_sensor{_steering_sensor} {};
...
// Calculates the Velocity [in RPM] of each wheel during a turn, W1 = [LEFT WHEEL] || W2 = [RIGHT WHEEL]
inline VelocityStruct get_Differential(float Steering_dg, float Apps){
    int64_t RPM_W1,RPM_W2;
    float Ack_rad;
    float del_W;
    VelocityStruct Differential_vel;
...
//* Calculates the Velocity [in RPM] of each wheel during a turn, W1 = [LEFT WHEEL] || W2 = [RIGHT WHEEL] *//
void ElectronicDifferential(float Steering_dg, uint16_t Wv_rpm, float apps, float Wc_Motor[]){
    int64_t RPM_W1,RPM_W2;
    float Ack_rad;
    float del_W;
    int64_t MAX_RPM; 
